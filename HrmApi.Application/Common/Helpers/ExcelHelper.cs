...
namespace HrmApi.Application.Common.Helpers
{
    public static class ExcelHelper
    {
            ...
            IEnumerable<(string Code, string Name)> items)
        {
            ...
            int row = 2;
            foreach ((string Code, string Name) in items)
            {
                IXLCell c1 = ws.Cell(row, 1);
                c1.Value = Code ?? string.Empty;
        ...
        public static DateTime? ParseDate(IXLCell cell)
        {
            ...
            return DateTime.TryParse(text, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime parsed)
        ...
        public static int? ParseInt(IXLCell cell)
        {
            ...
            return cell.TryGetValue(out int intValue) ? intValue : int.TryParse(cell.GetString(), out int parsed) ? parsed : null;
        }
        ...
        public static decimal? ParseDecimal(IXLCell cell)
        {
            ...
            if (cell.TryGetValue(out double doubleVal))
            {
                return (decimal)doubleVal;
            }

            if (cell.TryGetValue(out decimal decVal))
            {
                return decVal;
            }

            string text = cell.GetString().Trim().Replace(",", ".");
            return decimal.TryParse(text, NumberStyles.Any, CultureInfo.InvariantCulture, out decimal parsed) ? parsed : null;
        }

        public static DateOnly? ParseDateOnly(IXLCell cell)
        {
            DateTime? dt = ParseDate(cell);
