...
namespace HrmApi.Application.Features.Timekeepings.Queries
{
    ...
    public class GetTimekeepingsPagedQueryHandler : IRequestHandler<GetTimekeepingsPagedQuery, PagedResult<TimekeepingDto>>
    {
        ...
        public GetTimekeepingsPagedQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            IQueryable<TimekeepingEntity> query = _context.TimekeepingEntities.AsNoTracking();
            ...
            List<TimekeepingEntity> entities = await query
                .Skip((request.PageIndex - 1) * request.PageSize)
                .Take(request.PageSize)
                .ToListAsync(cancellationToken);

            List<TimekeepingDto> items = await MapAsync(entities, cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            Dictionary<Guid, string> shiftMasters = smIds.Count == 0
                ? []
            ...
            return entities.Select(x =>
            {
                ...
                string? smName = x.ShiftMasterId.HasValue && shiftMasters.TryGetValue(x.ShiftMasterId.Value, out string? sn) ? sn : null;
    ...
    {
        private readonly IApplicationDbContext _context;
        public GetTimekeepingByIdQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            TimekeepingEntity? entity = await _context.TimekeepingEntities.AsNoTracking()
    ...
    {
        private readonly IApplicationDbContext _context;
        public GetTimekeepingSummariesPagedQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            IQueryable<TimekeepingSummaryEntity> query = _context.TimekeepingSummaryEntities.AsNoTracking().Where(x => !x.IsDeleted);
            ...
            List<TimekeepingSummaryEntity> entities = await query
                .Skip((request.PageIndex - 1) * request.PageSize)
                .Take(request.PageSize)
                .ToListAsync(cancellationToken);
