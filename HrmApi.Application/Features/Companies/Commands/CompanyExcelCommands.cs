    ...
    public class ImportCompaniesExcelCommandHandler : IRequestHandler<ImportCompaniesExcelCommand, CompanyImportResultDto>
    {
        ...
        private static CompanyCommandFields ReadRow(IXLRangeRow row)
        {
            return new CompanyCommandFields
            {
                ...
                Ward = GetCellString(row, 12),
                BusinessRegistrationCode = GetCellString(row, 13),
                FoundedDate = ParseDate(row.Cell(14)),
                OperatingStatus = GetCellString(row, 15),
                LegalRepresentative = GetCellString(row, 16),
                LegalRepresentativePosition = GetCellString(row, 17),
                CompanyType = GetCellString(row, 18),
                Industry = GetCellString(row, 19),
                BankAccountNumber = GetCellString(row, 20),
                BankName = GetCellString(row, 21),
                BankBranch = GetCellString(row, 22),
                PrefixMaleCode = GetCellString(row, 23),
                PrefixFemaleCode = GetCellString(row, 24),
                PrefixFullTimeCode = GetCellString(row, 25),
                PrefixPartTimeCode = GetCellString(row, 26),
                DayComputeSalary = ParseDate(row.Cell(27)),
                IsComputePrevMonth = ParseBool(row.Cell(28)),
                TimeZone = GetCellString(row, 29),
                DefaultLanguage = GetCellString(row, 30),
                LogoUrl = GetCellString(row, 31),
                IsActive = ParseBool(row.Cell(32)) ?? true,
                SocialInsuranceCode = GetCellString(row, 33)
            };
        }
