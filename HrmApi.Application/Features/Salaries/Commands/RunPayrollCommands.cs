...
namespace HrmApi.Application.Features.Salaries.Commands
{
    ...
    public class PreviewPayrollRunQueryHandler : IRequestHandler<PreviewPayrollRunQuery, PayrollPreviewResultDto>
    {
        ...
        public PreviewPayrollRunQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            PayrollRunEngine engine = new(_context);
    ...
    public class RunPayrollCommandHandler : IRequestHandler<RunPayrollCommand, PayrollRunResultDto>
    {
        ...
        public async Task<PayrollRunResultDto> Handle(RunPayrollCommand request, CancellationToken cancellationToken)
        {
            ...
            PayrollRunResultDto result = new();
            ...
            foreach (PayrollPreviewItemDto item in preview.Items.Where(x => x.CanCreate))
            {
                if (item.ExistingSalaryId.HasValue)
                {
                    ...
                    foreach (SalaryLineItemEntity? old in existing.LineItems.Where(x => !x.IsDeleted).ToList())
                    {
                        old.IsDeleted = true;
                        old.UpdatedAt = DateTime.UtcNow;
                    }
                    foreach (SalaryLineItemEntity line in BuildEntitiesFromDto(item.LineItems))
                    {
    ...
    public class FinalizePayrollPeriodCommandHandler : IRequestHandler<FinalizePayrollPeriodCommand, int>
    {
        ...
        public async Task<int> Handle(FinalizePayrollPeriodCommand request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<SalaryEntity> query = _context.SalaryEntities
                .Include(x => x.LineItems)
                .Where(x => !x.IsDeleted
                    && x.Year == request.Year
            ...
            foreach (SalaryEntity salary in salaries)
            {
            ...
            List<AdvanceEntity> advances = await _context.AdvanceEntities
                .Where(x => !x.IsDeleted
            ...
            foreach (AdvanceEntity? a in advances)
            {
                a.Status = SlipStatus.Applied;
                a.UpdatedAt = now;
            }

            List<DeductionSlipEntity> deductions = await _context.DeductionSlipEntities
                .Where(x => !x.IsDeleted
                    && empIds.Contains(x.EmployeeId)
                    ...
                    && x.Status == SlipStatus.Approved)
                .ToListAsync(cancellationToken);
            foreach (DeductionSlipEntity? d in deductions)
            {
                d.Status = SlipStatus.Applied;
                d.UpdatedAt = now;
            }

            List<CashAdditionSlipEntity> additions = await _context.CashAdditionSlipEntities
                .Where(x => !x.IsDeleted
                    && empIds.Contains(x.EmployeeId)
                    && x.ApplyYear == request.Year
                    && x.ApplyMonth == request.Month
                    && x.Status == SlipStatus.Approved)
                .ToListAsync(cancellationToken);
            foreach (CashAdditionSlipEntity? c in additions)
            {
            ...
            await _actionLog.LogActionAsync(
                ...
                null,
                null,
                new { request.Year, request.Month, salaries.Count },
    ...
    internal sealed class PayrollRunEngine
    {
        ...
        public PayrollRunEngine(IApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<PayrollPreviewResultDto> PreviewAsync(PayrollRunRequest request, CancellationToken ct)
        {
            ...
            IQueryable<TimekeepingSummaryEntity> summariesQuery = _context.TimekeepingSummaryEntities.AsNoTracking()
            ...
            List<Guid> empIds = summaries.Select(x => x.EmployeeId).Distinct().ToList();
            ...
            Dictionary<Guid, SalaryEntity> existingSalaries = await _context.SalaryEntities.AsNoTracking()
            ...
            List<EmployeeSalaryHistoryEntity> salaryHistories = await _context.EmployeeSalaryHistoryEntities.AsNoTracking()
                .Where(x => !x.IsDeleted && empIds.Contains(x.EmployeeId) && x.EffectiveDate <= periodEndDt)
                .OrderByDescending(x => x.EffectiveDate)
                .ToListAsync(ct);
            Dictionary<Guid, EmployeeSalaryHistoryEntity> historyMap = salaryHistories
                .GroupBy(x => x.EmployeeId)
                .ToDictionary(g => g.Key, g => g.First());

            List<ContractEntity> contracts = await _context.ContractEntities.AsNoTracking()
                .Where(x => !x.IsDeleted && empIds.Contains(x.EmployeeId))
                .OrderByDescending(x => x.StartDate)
                .ToListAsync(ct);
            Dictionary<Guid, ContractEntity> contractMap = contracts
                .GroupBy(x => x.EmployeeId)
                .ToDictionary(g => g.Key, g => g.First());

            Dictionary<Guid, int> dependentCounts = await _context.EmployeeDependentEntities.AsNoTracking()
                .Where(x => !x.IsDeleted && empIds.Contains(x.EmployeeId)
            ...
            List<AdvanceEntity> advances = await _context.AdvanceEntities.AsNoTracking()
                .Where(x => !x.IsDeleted && empIds.Contains(x.EmployeeId)
                    && x.DeductYear == request.Year && x.DeductMonth == request.Month
                    && (x.Status == SlipStatus.Approved || x.Status == SlipStatus.Applied))
                .ToListAsync(ct);

            List<DeductionSlipEntity> deductions = await _context.DeductionSlipEntities.AsNoTracking()
                .Where(x => !x.IsDeleted && empIds.Contains(x.EmployeeId)
                    && x.ApplyYear == request.Year && x.ApplyMonth == request.Month
                    && (x.Status == SlipStatus.Approved || x.Status == SlipStatus.Applied))
                .ToListAsync(ct);

            List<CashAdditionSlipEntity> additions = await _context.CashAdditionSlipEntities.AsNoTracking()
                .Where(x => !x.IsDeleted && empIds.Contains(x.EmployeeId)
                    && x.ApplyYear == request.Year && x.ApplyMonth == request.Month
                    && (x.Status == SlipStatus.Approved || x.Status == SlipStatus.Applied))
                .ToListAsync(ct);
            ...
            List<Guid> branchIds = employees.Values.Where(e => e.BranchId.HasValue).Select(e => e.BranchId!.Value).Distinct().ToList();
            ...
            List<Guid> companyIds = employees.Values.Where(e => e.CompanyId.HasValue).Select(e => e.CompanyId!.Value).Distinct().ToList();
            List<SalaryConfigEntity> configs = await _context.SalaryConfigEntities.AsNoTracking()
            ...
            PayrollPreviewResultDto result = new()
            {
                Year = request.Year,
                Month = request.Month,
            ...
            foreach (TimekeepingSummaryEntity summary in summaries)
            {
                if (!employees.TryGetValue(summary.EmployeeId, out EmployeeEntity? emp))
                {
                    continue;
                }

                PayrollPreviewItemDto item = new()
                {
                    EmployeeId = emp.Id,
                ...
                if (existingSalaries.TryGetValue(emp.Id, out SalaryEntity? existing))
                {
                ...
                decimal basicFull = 0;
                if (historyMap.TryGetValue(emp.Id, out EmployeeSalaryHistoryEntity? hist))
                {
                    basicFull = hist.NewBasicSalary;
                }
                else if (contractMap.TryGetValue(emp.Id, out ContractEntity? contract))
                {
                ...
                decimal basicPaid = Math.Round(basicFull * actualDays / standardDays, 0, MidpointRounding.AwayFromZero);
                ...
                int order = 10;
                foreach (AllowanceEntity? alw in allowances.Where(a => a.CompanyId == null || a.CompanyId == emp.CompanyId))
                {
                ...
                if (historyMap.TryGetValue(emp.Id, out EmployeeSalaryHistoryEntity? h2) && h2.Allowance is > 0)
                {
                    lines.Add(PayrollLineFactory.Income("ALLOWANCE", "Phụ cấp (lịch sử lương)", h2.Allowance.Value, order++));
                }

                foreach (CashAdditionSlipEntity? add in additions.Where(x => x.EmployeeId == emp.Id))
                {
                    lines.Add(PayrollLineFactory.Income(
                ...
                foreach (AdvanceEntity? adv in advances.Where(x => x.EmployeeId == emp.Id))
                {
                    lines.Add(PayrollLineFactory.Deduction(SalaryItemCode.Advance, "Tạm ứng", adv.Amount, 110, adv.Reason));
                }

                foreach (DeductionSlipEntity? ded in deductions.Where(x => x.EmployeeId == emp.Id))
                {
                ...
                {
                    _ = dependentCounts.TryGetValue(emp.Id, out int deps);
