...
namespace HrmApi.Application.Features.RegisterDayOffs.Commands
{
    ...
    {
        /// <summary>
        /// Admin có thể chỉ định EmployeeId; Mobile bỏ trống để lấy từ JWT.
        /// </summary>
    ...
    public class CreateRegisterDayOffCommandHandler : IRequestHandler<CreateRegisterDayOffCommand, Guid>
    {
        ...
        public async Task<Guid> Handle(CreateRegisterDayOffCommand request, CancellationToken cancellationToken)
        {
            ...
            string dayOffType = request.DayOffType ?? DayOffType.Annual;
            if (request.DayOffConfigId.HasValue)
            {
                var config = await _context.DayOffConfigEntities
            ...
            var overlap = await _context.RegisterDayOffEntities
            ...
            var totalDays = request.ToDate.DayNumber - request.FromDate.DayNumber + 1;
            ...
            _context.RegisterDayOffEntities.Add(entity);
    ...
    public class ApproveRegisterDayOffCommandHandler : IRequestHandler<ApproveRegisterDayOffCommand, bool>
    {
        ...
        {
            var entity = await _context.RegisterDayOffEntities
                .FirstOrDefaultAsync(x => x.Id == request.Id && !x.IsDeleted, cancellationToken);
            if (entity == null) return false;
            if (entity.Status != DayOffStatus.Pending)
                throw new InvalidOperationException("Chỉ duyệt đơn đang chờ duyệt.");

            var oldValue = RegisterDayOffMapper.ToLogObject(entity);
            entity.Status = DayOffStatus.Approved;
            entity.ApproverId = _currentUser.UserId;
            entity.ApprovedAt = DateTime.UtcNow;
            entity.ApproverNote = string.IsNullOrWhiteSpace(request.ApproverNote) ? null : request.ApproverNote.Trim();
            entity.UpdatedAt = DateTime.UtcNow;

            // Mark overlapping timekeeping days as LEAVE
            for (var d = entity.FromDate; d <= entity.ToDate; d = d.AddDays(1))
            {
                var tk = await _context.TimekeepingEntities
                    .FirstOrDefaultAsync(x => x.EmployeeId == entity.EmployeeId && x.WorkDate == d && !x.IsDeleted, cancellationToken);
                if (tk != null)
                {
                ...
                else
                {
                    ...
                    var window = await _rules.ResolveWorkWindowAsync(employee, d, cancellationToken);
                    _context.TimekeepingEntities.Add(new Domain.Entities.Timekeeping.TimekeepingEntity
                    {
