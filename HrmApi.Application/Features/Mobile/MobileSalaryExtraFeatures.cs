...
namespace HrmApi.Application.Features.Mobile
{
    ...
    public class GetMyPayslipHtmlQueryHandler : IRequestHandler<GetMyPayslipHtmlQuery, MobilePayslipHtmlDto>
    {
        ...
        public async Task<MobilePayslipHtmlDto> Handle(GetMyPayslipHtmlQuery request, CancellationToken cancellationToken)
        {
            ...
            SalaryDto detail = await _mediator.Send(new GetMySalaryDetailQuery
            {
                Id = request.Id,
                Year = request.Year,
                Month = request.Month,
            ...
            _ = sb.Append("<!DOCTYPE html><html><head><meta charset='utf-8'><title>Payslip ")
            ...
            _ = sb.Append("<h2>Phiếu lương ").Append(WebUtility.HtmlEncode(detail.PeriodCode)).Append("</h2>");
            _ = sb.Append("<p><strong>Nhân viên:</strong> ").Append(WebUtility.HtmlEncode(detail.EmployeeName ?? ""))
              .Append(" (").Append(WebUtility.HtmlEncode(detail.EmployeeCode ?? "")).Append(")</p>");
            _ = sb.Append("<p class='muted'>Trạng thái: ").Append(WebUtility.HtmlEncode(detail.Status))
              .Append(" · Công: ").Append(detail.ActualWorkingDays).Append("/").Append(detail.StandardWorkingDays).Append("</p>");
            _ = sb.Append("<table><thead><tr><th>Loại</th><th>Mã</th><th>Khoản mục</th><th class='right'>Số tiền</th></tr></thead><tbody>");
            foreach (SalaryLineItemDto? line in detail.LineItems.OrderBy(x => x.DisplayOrder))
            {
                _ = sb.Append("<tr><td>").Append(WebUtility.HtmlEncode(line.ItemType)).Append("</td><td>")
            ...
            _ = sb.Append("<p><strong>Gross:</strong> ").Append(detail.GrossSalary.ToString("N0"))
            ...
            _ = sb.Append("<p class='muted'>In trang này để lưu PDF.</p></body></html>");
