...
namespace HrmApi.Application.Features.OvertimeRequests
{
    ...
    public class CreateOvertimeRequestCommandHandler : IRequestHandler<CreateOvertimeRequestCommand, Guid>
    {
        ...
        public async Task<Guid> Handle(CreateOvertimeRequestCommand request, CancellationToken cancellationToken)
        {
            ...
            OvertimeRequestEntity entity = new()
            {
            ...
            _ = _context.OvertimeRequestEntities.Add(entity);
            ...
            if (request.Submit)
            {
                ...
                _ = await _notificationService.NotifyAdminsAsync(
        ...
        internal static int ResolveRequestedMinutes(TimeSpan from, TimeSpan to, int? overrideMinutes)
        {
            ...
            {
                return overrideMinutes.Value <= 0 ? throw new InvalidOperationException("Số phút OT phải > 0.") : overrideMinutes.Value;
            }
            ...
            return minutes <= 0 ? throw new InvalidOperationException("Khung giờ OT không hợp lệ.") : minutes;
        }
    }
    #endregion
    ...
    public class SubmitOvertimeRequestCommandHandler : IRequestHandler<SubmitOvertimeRequestCommand, bool>
    {
        ...
        {
            OvertimeRequestEntity? entity = await _context.OvertimeRequestEntities
    ...
    public class ReviewOvertimeRequestCommandHandler : IRequestHandler<ReviewOvertimeRequestCommand, bool>
    {
        ...
        private async Task EnsureScheduleForDayOffOtAsync(OvertimeRequestEntity ot, CancellationToken cancellationToken)
        {
            ...
            EmployeeEntity employee = await _rules.ResolveEmployeeAsync(ot.EmployeeId, cancellationToken);
            WorkWindowResult window = await _rules.ResolveWorkWindowAsync(employee, ot.WorkDate, cancellationToken);
    ...
    public class BulkApproveOvertimeRequestCommandHandler : IRequestHandler<BulkApproveOvertimeRequestCommand, int>
    {
        ...
        public BulkApproveOvertimeRequestCommandHandler(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        : IRequestHandler<GetOvertimeRequestsPagedQuery, PagedResult<OvertimeRequestDto>>
    {
        ...
        public GetOvertimeRequestsPagedQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<PagedResult<OvertimeRequestDto>> Handle(
            ...
            CancellationToken cancellationToken)
        {
            IQueryable<OvertimeRequestEntity> query = _context.OvertimeRequestEntities.AsNoTracking().Where(x => !x.IsDeleted);
            ...
            List<OvertimeRequestEntity> entities = await query
                .OrderByDescending(x => x.CreatedAt)
                .Skip((request.PageIndex - 1) * request.PageSize)
                .Take(request.PageSize)
                .ToListAsync(cancellationToken);

            List<OvertimeRequestDto> dtos = await OvertimeRequestQueryHelper.MapManyAsync(_context, entities, cancellationToken);
    ...
    public class GetOvertimeRequestByIdQueryHandler : IRequestHandler<GetOvertimeRequestByIdQuery, OvertimeRequestDto?>
    {
        ...
        {
            OvertimeRequestEntity? entity = await _context.OvertimeRequestEntities.AsNoTracking()
                .FirstOrDefaultAsync(x => x.Id == request.Id && !x.IsDeleted, cancellationToken);
            if (entity == null)
            {
                return null;
            ...
            List<OvertimeRequestDto> list = await OvertimeRequestQueryHelper.MapManyAsync(_context, [entity], cancellationToken);
    ...
    internal static class OvertimeRequestQueryHelper
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            List<Guid> empIds = entities.Select(x => x.EmployeeId)
